...
fn main() {
    ...
    println!("cargo:rerun-if-env-changed=RPPM_VERSION");
    println!("cargo:rerun-if-env-changed=GEN_ARTIFACTS");

//     if option_env!("RPPM_VERSION").is_none() {
//         println!("cargo:rustc-env=RPPM_VERSION={}", rppm_version());
//     }
...
//         Man::new(cmd.clone())
//             .render(&mut File::create(out.join("rppm.1")).unwrap())
//             .unwrap();

//         for subcmd in cmd.get_subcommands() {
//             let name = format!("rppm-{}", subcmd.get_name());
...
//             generate_to(*shell, cmd, "rppm", out).unwrap();
//         }
//     }
}

// /// Also used by `args.rs`.
// fn rppm_version() -> String {
//     if let Some(version) = option_env!("RPPM_VERSION") {
